...
def run(hand_tracker, model):
    ...
    while True:
        ...
        frame_nn = np.array(frame_nn, dtype=np.float32)[None,]
        ...
        hand_tracker.palm_interpreter.set_tensor(hand_tracker.pd_input_index, frame_nn)
        hand_tracker.palm_interpreter.invoke()
        inference_1 = hand_tracker.palm_interpreter.get_tensor(hand_tracker.pd_output_index_1)
        inference_2 = hand_tracker.palm_interpreter.get_tensor(hand_tracker.pd_output_index_2)
        inference = {hand_tracker.pd_scores: inference_1, hand_tracker.pd_bboxes: inference_2}
        ...
        if hand_tracker.use_lm:
            for i, r in enumerate(hand_tracker.regions):
                ...
                # Get hand landmarks
                lm_rtrip_time = now()
                hand_tracker.landmark_interpreter.set_tensor(hand_tracker.lm_input_index, frame_nn)
                hand_tracker.landmark_interpreter.invoke()
                inference_1 = hand_tracker.landmark_interpreter.get_tensor(hand_tracker.lm_output_index_1)
                inference_2 = hand_tracker.landmark_interpreter.get_tensor(hand_tracker.lm_output_index_2)
                inference_3 = hand_tracker.landmark_interpreter.get_tensor(hand_tracker.lm_output_index_3)
                inference = {hand_tracker.lm_score: inference_1, hand_tracker.lm_handedness: inference_2, hand_tracker.lm_landmarks: inference_3}
        
                glob_lm_rtrip_time += now() - lm_rtrip_time
                nb_lm_inferences += 1
                hand_tracker.lm_postprocess(r, inference)
                hand_tracker.lm_render(annotated_frame, r)
        inference_end = time.time_ns() // 1000000

        inference_time_sum += inference_end - inference_start
        inference_num += 1
