...
class HandTracker:
    ...
    def load_models(self):
        ...
        palm_model_path = os.path.join("..", "MediaPipe", "mediapipe_models", "palm_detection_full.tflite")

        self.palm_interpreter = tf.lite.Interpreter(model_path=palm_model_path)
        self.palm_interpreter.allocate_tensors()
        
        # for GPU inference
        self.palm_interpreter.set_tensor( palm_interpreter.get_delegate().set_gpu_compatibility(True) )

        print(self.palm_interpreter.get_input_details())
        print(self.palm_interpreter.get_output_details())

        # Input blob: input_1 - shape: [1, 3, 192, 192]
        # Output blob: Identity - shape: [1, 2016, 18]
        # Output blob: Identity_1 - shape: [1, 2016, 1]
        self.pd_input_blob = next(iter(self.palm_interpreter.get_input_details()))["name"]
        self.pd_input_index = next(iter(self.palm_interpreter.get_input_details()))["index"]

        input_shape = next(iter(self.palm_interpreter.get_input_details()))["shape"]
        print(f"Input blob: {self.pd_input_blob} - shape: {input_shape}")
        _, self.pd_w, self.pd_h, _ = (next(iter(self.palm_interpreter.get_input_details()))["shape"])

        self.pd_output_index_1 = self.palm_interpreter.get_output_details()[1]["index"]
        self.pd_output_index_2 = self.palm_interpreter.get_output_details()[0]["index"]
        ...
        # Landmarks model
        if self.use_lm:
            landmark_model_path = os.path.join("..", "MediaPipe", "mediapipe_models", "hand_landmark_full.tflite")

            self.landmark_interpreter = tf.lite.Interpreter(model_path=landmark_model_path)
            self.landmark_interpreter.allocate_tensors()

            # for GPU inference
            self.landmark_interpreter.set_tensor( landmark_interpreter.get_delegate().set_gpu_compatibility(True) )

            print(self.landmark_interpreter.get_input_details())
            print(self.landmark_interpreter.get_output_details())
            ...
            self.lm_input_blob = next(iter(self.landmark_interpreter.get_input_details()))["name"]
            self.lm_input_index = next(iter(self.landmark_interpreter.get_input_details()))["index"]
            input_shape = next(iter(self.landmark_interpreter.get_input_details()))["shape"]
            print(f"Input blob: {self.lm_input_blob} - shape: {input_shape}")
            
            _, self.lm_w, self.lm_h, _ = (next(iter(self.landmark_interpreter.get_input_details()))["shape"])

            self.lm_output_index_1 = self.landmark_interpreter.get_output_details()[1]["index"]
            self.lm_output_index_2 = self.landmark_interpreter.get_output_details()[2]["index"]
            self.lm_output_index_3 = self.landmark_interpreter.get_output_details()[0]["index"]
    ...
    def run(self):
        ...
        while True:
            ...
            self.palm_interpreter.set_tensor(self.pd_input_index, frame_nn)
            self.palm_interpreter.invoke()
            inference = self.palm_interpreter.get_tensor(self.pd_output_index)
            ...
            if self.use_lm:
                for i, r in enumerate(self.regions):
                    ...
                    self.landmark_interpreter.set_tensor(self.lm_input_index, frame_nn)
                    self.landmark_interpreter.invoke()
                    inference = self.landmark_interpreter.get_tensor(self.lm_output_index)
